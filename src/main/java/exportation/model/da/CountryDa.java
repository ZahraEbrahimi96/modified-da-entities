...
public class CountryDa implements AutoCloseable, CRUD<Country> {
    ...
    public Country edit(Country country) throws Exception {
        preparedStatement = connection.prepareStatement(
                "UPDATE COUNTRY SET COUNTRY_NAME=?, COUNTRY_PHONE_CODE=?, COUNTRY_SUPPLIER=?,COUNTRY_MANUFACTURER=?,COUNTRY_INFO=? WHERE COUNTRY_ID=?"
        );
        ...
        preparedStatement.setString(5, String.valueOf(country.getInfo()));
        preparedStatement.setInt(6, country.getId());
        preparedStatement.execute();
        return country;
    }
